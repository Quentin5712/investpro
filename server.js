...
// Chercher index.html à la racine OU dans public/
const PUBLIC   = fs_root.existsSync(path.join(__dirname, 'public', 'index.html'))
...
app.get('/api/ping', function(req, res) { res.json({ ok: true, msg: 'InvestPro API OK' }); });
...
app.get('*', function(req, res) {
  ...
  else { res.status(200).json({ ok: true, msg: 'InvestPro API OK — index.html non trouvé' }); }
});

// ── Start ─────────────────────────────────────────────────
app.listen(PORT, '0.0.0.0', function() {
  console.log('✅ InvestPro running on port ' + PORT);
  console.log('📁 Public: ' + PUBLIC);
  console.log('📄 index.html: ' + (fs.existsSync(path.join(PUBLIC,'index.html')) ? '✅ OK' : '❌ INTROUVABLE'));
});
