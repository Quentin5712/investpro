...
function serveHTML(res) {
  ...
  fs.readFile(file, function(err, data) {
    if (err) {
      res.writeHead(500); res.end('index.html introuvable'); return;
    }
    ...
    const req = https.request(opts, function(res) {
      ...
      res.on('end', function() {
        try {
          ...
          resolve(j && j.record !== undefined ? j.record : j);
        } catch(e) { resolve(null); }
      });
    });
    req.on('error', function(e) { console.error('dbRead error:', e.message); resolve(null); });
    req.end();
  });
}
...
function dbWrite(data) {
  return new Promise(function(resolve) {
    ...
    req.on('error', function(e) { console.error('dbWrite error:', e.message); resolve(false); });
    req.write(body);
    req.end();
  });
}
...
function parseSMS(txt) {
  ...
  for (let i = 0; i < P.length; i++) {
    const p = P[i];
    const am = txt.match(p.amt), te = txt.match(p.tel), co = txt.match(p.code);
...
http.createServer(async function(req, res) {
  ...
  if (req.method === 'GET' && !route.startsWith('/api')) {
    if (route === '/' || route === '/index.html') { serveHTML(res); return; }
    if (route === '/sms-listener') { serveStatic(res, path.join(__dirname, 'sms-listener.html')); return; }
    // Toute autre URL GET → servir le HTML (SPA fallback)
    serveHTML(res); return;
  }
  ...
  if (route === '/api/register' && req.method === 'POST') {
    if (!b.name || !b.email || !b.phone || !b.pass) { send(res, 400, { error: 'Champs manquants' }); return; }
    const db = await dbRead();
    if (!db) { send(res, 500, { error: 'Erreur base de données' }); return; }
    if (!db.siteOpen) { send(res, 403, { error: 'Inscriptions fermées' }); return; }
    if ((db.users||[]).find(function(u) { return u.email === b.email.toLowerCase(); })) { send(res, 409, { error: 'Email déjà utilisé' }); return; }
  ...
  if (route === '/api/login' && req.method === 'POST') {
    if (b.email === 'quentin' && b.pass === 'Quentin') { send(res, 200, { ok: true, user: { id: 'admin', name: 'Quentin', email: 'admin@investpro', isAdmin: true } }); return; }
    const db = await dbRead();
    if (!db) { send(res, 500, { error: 'Erreur base de données' }); return; }
  ...
  if (route === '/api/user' && req.method === 'GET') {
    const db = await dbRead();
    if (!db) { send(res, 500, { error: 'Erreur BDD' }); return; }
    const u = (db.users||[]).find(function(x) { return x.email === parsed.query.email; });
  ...
  if (route === '/api/withdraw' && req.method === 'POST') {
    ...
    if (!u || (u.withdrawBalance||0) < b.amount) { send(res, 400, { error: 'Solde insuffisant' }); return; }
  ...
  if (route === '/api/admin/data' && req.method === 'GET') {
    ...
    if (!db) { send(res, 500, { error: 'Erreur BDD' }); return; }
    send(res, 200, {
      users:         (db.users||[]).map(function(u) { const s=Object.assign({},u);delete s.pass;return s; }),
      deposits:      db.deposits||[],
      withdrawals:   db.withdrawals||[],
      notifications: db.notifications||[],
      siteOpen:      db.siteOpen
    }); return;
  }
  ...
  if (route === '/api/admin/sms-verify' && req.method === 'POST') {
    if (b.adminKey !== ADMIN_KEY) { send(res, 403, { error: 'Clé invalide' }); return; }
    const sms = parseSMS(b.smsText||'');
    if (!sms) { send(res, 200, { ok: false, message: 'SMS non reconnu' }); return; }
    const db = await dbRead(); if (!db) { send(res, 500, { error: 'Erreur BDD' }); return; }
  ...
  // Toute route API inconnue
  send(res, 404, { error: 'Route introuvable: ' + route });

}).listen(PORT, '0.0.0.0', function() {
  console.log('✅ InvestPro server on port ' + PORT);
  console.log('📁 Public dir: ' + PUBLIC);
  fs.access(path.join(PUBLIC, 'index.html'), function(err) {
    console.log('📄 index.html: ' + (err ? '❌ INTROUVABLE' : '✅ OK'));
  });
});
